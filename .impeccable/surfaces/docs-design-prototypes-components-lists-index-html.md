...
视觉：模块色按用户本轮指定的 `#176E5A`／`#A23F5D` 色相更新，中性明暗表面和字体沿用基础规范。全原型固定标准密度，桌面通用控件高 36px、行垂直内边距 12px，不提供密度切换或保存密度偏好；手机独立安排视觉尺寸与至少 44px 的触摸热区。值源仍为 `tokens.css`，不增加装饰性图像。

使用路径：搜索或筛选 → 单项／本页多选 → 复制名称或取消 → 翻页；切换范围清空选择。预览控制单独置于列表外。

首屏：桌面保留状态预览，表头与六列内容均有骨架；手机将主题、模块与状态收进顶栏“预览”，压缩标题和工具栏，先呈现列表卡片。手机使用选择条和卡片对应的骨架，不留下空表头区域。手机搜索旁重置按钮与输入保持 40px 等高，独立复制按钮维持 32px 外观；纯图标居中，重置按钮以额外热区保障 44px 触摸范围。
