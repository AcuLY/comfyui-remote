...
视觉：模块色按用户本轮指定的 `#176E5A`／`#A23F5D` 色相更新，中性明暗表面和字体沿用基础规范。桌面密度只比较桌面高度与行距；手机独立安排视觉尺寸与至少 44px 的触摸热区。值源仍为 `tokens.css`，不增加装饰性图像。

使用路径：搜索或筛选 → 单项／本页多选 → 复制名称或取消 → 翻页；切换范围清空选择。预览控制单独置于列表外。

首屏：桌面保留状态／密度比较，表头与六列内容均有骨架；手机将主题、模块与状态收进顶栏“预览”，压缩标题和工具栏，先呈现列表卡片。手机使用选择条和卡片对应的骨架，不留下空表头区域。
