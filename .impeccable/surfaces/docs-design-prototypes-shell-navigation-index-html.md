...
本轮呈现：宽屏展开侧栏，窄屏默认图标栏并支持手动收放；按用户后续反馈，收起时模块使用图片／训练芯片图标，名称保留在悬停提示与无障碍标签；展开侧栏和手机顶部仍显示完整名称。手机显示完整四个入口，更多使用原生模态对话框的右侧呈现。活动入口同时有图标、名称和非颜色标识。占位内容明确不是业务页面，便于评审可用空间。
