...
import { createThemeInputs, themeDensity } from '../theme/theme-inputs.mjs';

const project = fileURLToPath(new URL('../', import.meta.url));
const upstream = path.join(project, 'theme/vendor/primereact-sass-theme');
const tokensFile = path.join(project, 'theme/tokens.css');
const output = path.join(project, 'src/theme.generated.css');
...
${scope} {
  ...
  @layer primereact {
    ...
    /* 内边距让自然高度落在 36/40px 密度上，min-height 只用于补偿缩放下的边框量化；
       两者同时成立时 min-height 不产生多余空隙，标签仍居中。 */
...
`;
  const result = sass.compileString(source, {
    url: pathToFileURL(path.join(project, `theme/${variant.theme}-${variant.module}.scss`)),
...
async function main() {
  ...
  const rules = tokenRules(await readFile(tokensFile, 'utf8'));
  const blocks = [];
  for (const touch of [false, true]) {
    for (const theme of ['light', 'dark']) {
      for (const module of ['production', 'training']) blocks.push(await compileTheme(rules, { theme, module, touch }));
    }
  }
  const css = `/*! Generated by scripts/build-theme.mjs. Do not edit.
 * PrimeReact Sass theme 10.8.5, commit ${sourceCommit}.
 * Copyright (c) 2023 PrimeTek. MIT; see theme/LICENSE.primereact-sass-theme.
 * Semantic inputs: theme/tokens.css; mappings: theme/theme-inputs.mjs.
 */
${blocks.join('\n')}
`;
  ...
  if (args[0] === '--check') {
    ...
    console.log(`Generated src/theme.generated.css (${size}).`);
  }
}
