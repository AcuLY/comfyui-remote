...
| 组件和输入文字 | 14px | 16px |
| 统一行高 | 1.5 | 1.5 |
| 输入、文字按钮内边距 | 6.5px 12px | 7px 12px |
| 带 1px 边框的输入／文字按钮 | 36px | 40px |
| 纯图标按钮 | 36px | 44px |
| 关闭等动作图标 | 36px | 44px |
| 分页动作 | 36px | 44px |
| 无边框菜单项 | 36px | 44px |

表中数值以浏览器 16px 根字号为基准。官方源会将尺寸与固定 `rem` 相加，`Sass` 输入因此使用等值数值 `rem`，允许用户根字号缩放；不能直接把这些参与乘法、`nth()` 和颜色运算的输入改成 CSS `var()`。主题入口为文字按钮、实际输入、下拉和多选根节点设置桌面36px、手机／粗指针40px最小高度，补偿浏览器缩放时的边框量化；纯图标按钮、关闭及分页动作保留真实44px，不添加透明热区，也不向下拉内部标签重复设置高度。数值输入等复合组件保留库原生按钮布局，整体高度按其组合实测，不强制所有复合控件等高40px。进度条通过官方 `$progressBarHeight` 设为 6px，文字在条外呈现。复选框、单选框与开关保留官方结构及标记尺寸，关联标签的操作行由组件组合负责。

列表组合通过官方 `$tableHeaderCellPadding`、`$tableBodyCellPadding` 设置 12px 垂直内边距，表头字重通过 `$tableHeaderCellFontWeight` 设为 600；列宽仍由 `Column` 公开属性和组合布局负责。
