...
| --- | --- |
| 按钮 | `Button` |
| 状态徽标 | `Tag`（业务语义用 `severity` 属性，柔和底色走组件提供器的 `pt` 根槽位） |
| 面板 | `Panel` |
| 搜索字段 | `InputText` + `IconField` / `InputIcon` |
| 悬浮选择 | `Dropdown` |
...
界面文字使用自托管的 `HarmonyOS Sans SC`（`--font-sans`）；任务编号、生成数量、耗时与时间等数字／参数使用 `IBM Plex Mono`（`--font-mono`，`.mono` 工具类）。等宽只作用于数字与标识符片段，不整句套用：中文（例如“今天”）始终留在界面字体上，避免一行内混排两套字体造成光学错位。

控件高度由主题内边距与字体行盒相加得到（普通控件约 `36px`、触控约 `40px`）；生成主题同时保留 `min-height` 兜底，用于补偿浏览器缩放时的边框量化，两者一致时不产生额外空隙。
