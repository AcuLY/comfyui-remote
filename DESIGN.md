...
本文件只保留功能、交互、无障碍与运行时职责边界。上一轮应用原型已删除，原有设计风格及其确认记录不再作为后续设计依据。2026-09-16 按用户要求使用 PrimeReact 重新制作[新版应用完整原型](prototypes/app/README.md)（独立原型目录、独立依赖，不 import 应用本体），当前待用户评审；这不代表正式界面已获批准或实施。功能规划保留在 [设计文档](docs/design/README.md) 中。
