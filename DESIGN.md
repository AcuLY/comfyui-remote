...
本文件只保留功能、交互、无障碍与运行时职责边界。2026-09-16 按用户要求使用 `PrimeReact` 重新制作[新版应用完整原型](prototypes/app/README.md)（独立原型目录、独立依赖，不 `import` 应用本体），当前待用户评审；组件库版本与非侵入式定制边界见[新版前端基础规范](docs/design/prototypes/design-foundations.md)，视觉风格内容不再作为设计依据。这不代表正式界面已获批准或实施。功能规划保留在 [设计文档](docs/design/README.md) 中。
