...
本文件只保留功能、交互、无障碍与运行时职责边界。上一轮应用原型已删除，原有设计风格及其确认记录不再作为后续设计依据。2026-09-16 按用户要求沿用 `design-demos` 风格重新制作[新版应用完整原型](src/app/prototype/README.md)（应用侧栏与任务页，复用共享设计外壳与组件库），当前待用户评审；这不代表正式界面已获批准或实施。功能规划保留在 [设计文档](docs/design/README.md) 中。
