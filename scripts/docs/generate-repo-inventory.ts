...
const output = [
  ...
  "    kind: reference",
  "  readWhen:",
  "    - inspecting tracked repository paths and ownership",
  "  sources:",
...
if (args.includes("--check")) {
  const current = existsSync(INVENTORY_PATH) ? readFileSync(INVENTORY_PATH, "utf8").replaceAll("\r\n", "\n") : null;
  if (current !== output) {
    console.error(`${INVENTORY_PATH} is stale; run npx tsx scripts/docs/generate-repo-inventory.ts to regenerate it.`);
    process.exitCode = 1;
  }
} else {
