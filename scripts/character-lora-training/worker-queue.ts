...
const HELP = `
Character LoRA worker queue supervisor
...
Options:
  --worker-owner-prefix <name>  Prefix for per-worker lease owners. Default: character-lora-queue.
  ...
  --benchmark-skip-wait        Pass --skip-wait to benchmark worker.
  --benchmark-timeout-ms <ms>  Pass --timeout-ms to benchmark worker.
...
Notes:
  ...
  process when queued Character LoRA tasks should be consumed.
`.trim();

main().catch((error) => {
  console.error("[character-lora worker-queue] failed");
...
async function main() {
  const cli = parseWorkerCli(process.argv.slice(2), {
    workerOwner: "character-lora-queue",
  });
  if (cli.help) {
    console.log(HELP);
    return;
  ...
  console.log("[character-lora worker-queue] starting", {
  ...
  const start = (spec: WorkerSpec) => {
    ...
    child.on("exit", (code, signal) => {
      ...
      console.error("[character-lora worker-queue] child exited", {
        worker: spec.key,
        code,
        signal,
        restartDelayMs,
  ...
  const shutdown = async (signal: NodeJS.Signals) => {
    ...
    stopping = true;
    console.log("[character-lora worker-queue] stopping", { signal });
...
function buildWorkerSpecs(values: Map<string, string | true>): WorkerSpec[] {
  ...
  const benchmarkTimeoutMs = readNumberOption(values, "--benchmark-timeout-ms");
  ...
  return [
    ...
    {
      key: "benchmark",
      script: "benchmark-worker.ts",
      ownerSuffix: "benchmark-worker",
      skipFlag: "--skip-benchmark",
      extraArgs: [
