...
export function SectionCard({ title, subtitle, actions, children, className }: { title: ReactNode; subtitle?: string; actions?: ReactNode; children: ReactNode; className?: string }) {
  return (
    <section className={`-mx-4 w-full min-w-0 rounded-2xl border border-white/10 bg-[var(--panel)] p-3.5 shadow-[0_10px_30px_rgba(0,0,0,0.25)] sm:mx-0 lg:p-4${className ? ` ${className}` : ""}`}>
