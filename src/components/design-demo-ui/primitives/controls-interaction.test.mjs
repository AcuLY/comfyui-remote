...
const designDemoUiDir = resolve(testDir, "..");
const fieldSource = readDemoSource("primitives/field/index.tsx");
const floatingSelectSource = readDemoSource("primitives/floating-select/index.tsx");
const floatingSelectStyleSource = readDemoSource("primitives/floating-select/floating-select.module.css");
const segmentedControlStyleSource = readDemoSource("primitives/segmented-control/segmented-control.module.css");
const checkboxSource = readDemoSource("primitives/checkbox/index.tsx");
const buttonSource = readDemoSource("primitives/button/index.tsx");
const sortableSource = readDemoSource("primitives/sortable/index.tsx");

function readDemoSource(relativePath) {
  return readFileSync(resolve(designDemoUiDir, relativePath), "utf8");
}
...
test("legacy select and textarea adapter primitives are removed", () => {
  assert.equal(existsSync(resolve(designDemoUiDir, ["primitives/select", "like/index.tsx"].join("-"))), false);
  assert.equal(existsSync(resolve(designDemoUiDir, ["primitives/text", "area", "field/index.tsx"].join("-"))), false);
});
...
test("Button primitive icons stay decorative in the accessibility tree", () => {
  const iconRenderings = buttonSource.match(/<[^>]+data-demo-ui-button-icon="true"[^>]*\/>/g) ?? [];

  assert.equal(iconRenderings.length, 3, "Button, Button pending, and ButtonLink should each render one icon path");
  for (const rendering of iconRenderings) {
    assert.match(rendering, /aria-hidden="true"/, `${rendering} should hide the decorative icon`);
  }
});
