...
function serializeMarkdownReport(
  ...
  const lines: string[] = [
    "## Phase 0 历史基线",
    "",
    `- 通过：\`${verification.pass ? "true" : "false"}\``,
    `- 有效项目：${baseline.summary.validProjects}（${baseline.summary.validProjectTitles.join(", ")}）`,
    `- 已标注图片：${baseline.summary.labeledImages}`,
    `- 规范分区：${baseline.summary.canonicalSections}`,
    `- 已加载手动排除项：${baseline.summary.manualExclusionsLoaded.join(", ")}`,
    `- 已验证排序差异：\`${baseline.summary.sortOrderVarianceVerified}\``,
    `- 数据库已修改：\`${baseline.summary.dbMutated}\``,
    `- 可复现：\`${baseline.summary.reproducible}\``,
    "",
    "### 验收验证",
    "",
    "| 验收条件 | 状态 |",
