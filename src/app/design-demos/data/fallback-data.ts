...
export function fallbackData(warning: string | null): DemoData {
  ...
  const sections: DemoSection[] = ["角色草图", "场景光影", "最终组选"].map((name, index) => ({
    ...
    seedPolicy1: "random",
    seedPolicy2: "reuse",
    promptPositiveText: "cinematic portrait, soft lighting, refined detail",
    promptNegativeText: "low quality, blurry, extra fingers",
  ...
  const runs: DemoRun[] = [
    {
      ...
      executionMeta: {
        ...
        positivePrompt: sections[0].promptPositiveText,
        negativePrompt: sections[0].promptNegativeText,
      },
      images: demoImages,
    },
  ];
    ...
    templates: [
      {
        ...
        sections: sections.map((section) => ({
          ...
          notes: section.promptPositiveText,
        })),
      },
    ],
