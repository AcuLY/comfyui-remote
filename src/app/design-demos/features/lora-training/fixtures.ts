...
function buildDatasetSamples(images: DemoImage[], start: number, prefix: string): LoraTrainingDatasetSample[] {
  ...
  return picked.map((image, index) => ({
    ...
    image,
    caption: captions[index]?.text ?? `${prefix}, training caption snapshot`,
    status: index === 3 ? "pending" : "kept",
  }));
}

const defaultTrainingConfig = [
  { label: "Base", value: "sd_xl_base_1.0.safetensors", detail: "基础模型" },
  { label: "Network", value: "rank 16 / alpha 16", detail: "LoRA dim" },
  { label: "Steps", value: "2400", detail: "batch 2 · repeats 10" },
  { label: "LR", value: "1e-4", detail: "cosine scheduler" },
  { label: "Resolution", value: "1024", detail: "bucket enabled" },
  { label: "Runner", value: "local trainer", detail: "RTX worker" },
];
...
export function buildLoraTrainingDemoData(data: DemoData): LoraTrainingDemoData {
  ...
  const runs: LoraTrainingRun[] = [
    ...
    },
    {
      ...
      finalInput: "数据集 v5，42 张 kept 图片，LoRA rank 16，目标 2400 steps。",
      schedulerMessage: "训练已完成，final LoRA、配置和日志均已保存。",
      ...
      trainingLogLines: [
        ...
        "[16:31] step 2400 / 2400 · loss=0.071 · saving final LoRA",
    ...
    },
    {
      ...
      finalInput: "数据集 v4，33 张 kept 图片，LoRA rank 16，当前 step 1280 / 2400。",
    ...
    },
    {
      ...
      finalInput: "数据集 v3，51 张 kept 图片，等待本地训练 worker。",
    ...
    },
    {
      ...
      timestamp: "失败于 12:40",
      errorMessage: "训练脚本未找到冻结 manifest，请重新冻结数据集后再启动。",
      provider: "local trainer",
      datasetRevisionId: "草稿-current",
      finalInput: "数据集草稿，18 张 kept 图片，manifest 缺失。",
      schedulerMessage: "训练已停止，需先补齐缺失 caption 并重新冻结数据集。",
      trainingConfig: defaultTrainingConfig,
      trainingLogArtifactName: "training-run-noir-failed.log",
      trainingLogLines: [
        "[15:34] training started · dataset draft · 18 images",
        "[15:52] caption validation failed",
        "[15:55] stopped: 3 张图片缺少 caption",
      ],
