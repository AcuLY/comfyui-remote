...
**✅ 已完成**（2026-05-08）：CSS 数值已按 `DESIGN.md` 修正（`shell.css`：dark `saturate(145%→140%)`，light `--demo-glass-soft` `0.54→0.68`）。
...
| 类别 | 缺口严重程度 | 建议行动 | 状态 |
|-------|-------------|-----------|------|
| 布局模式未记录 | 中 | 在 Layout Rules 补充具体布局类说明 | 待处理 |
| 组件类型未记录 | 高 | 扩展 Section 6，按类别记录所有组件 | 待处理 |
| 响应式未充分记录 | 中 | 新增 Responsive Behavior 子章节 | 待处理 |
| 动效覆盖缺口 | 低 | 统一 hover 动效或更新文档 | 待处理 |
| 设计令牌数值偏差 | 低 | 统一数值或更新文档 | ✅ 已完成 |
