...
export function SyncPresetVariantFlowDialog({ projectId, projectTitle }: { projectId: string; projectTitle: string }) {
  ...
  const [sourceProjectTitle, setSourceProjectTitle] = useState("");
  const [targetProjectTitle, setTargetProjectTitle] = useState(projectTitle);
  ...
  async function requestFlow(
    ...
    const payload = buildSyncPresetVariantFlowPayload({
      sourceProjectTitle,
      targetProjectTitle,
      expectedSourceProjectId: expectedIds.expectedSourceProjectId,
      expectedTargetProjectId: expectedIds.expectedTargetProjectId,
      sourcePresetName,
      targetPresetName,
  ...
  async function handleDryRun() {
    ...
    try {
      const result = await requestFlow(true, { expectedTargetProjectId: projectId });
  ...
  async function handleApply() {
    ...
    const expectedSourceProjectId = dryRunResult.sourceProject?.id;
    const expectedTargetProjectId = dryRunResult.targetProject?.id;
    ...
    if (!confirm(`确认按「${sourceProjectTitle.trim()}」同步「${targetProjectTitle.trim()}」角色变体？计划切换 ${plannedUpdateCount} 段。`)) return;
      ...
      {isOpen && (
        <>
          ...
          <div
            ...
            className="fixed left-1/2 top-[50dvh] z-[110] flex max-h-[calc(100dvh-2rem)] w-[min(calc(100vw-2rem),44rem)] -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-xl border border-white/10 bg-zinc-950 shadow-2xl"
          >
            <div className="flex items-start justify-between gap-3 border-b border-white/10 p-3">
              <div className="min-w-0">
                ...
                <div className="mt-0.5 text-xs text-zinc-500">先 Dry Run 预览计划，再二次确认 Apply；目标默认当前项目。</div>
              </div>
              <button
                type="button"
                onClick={closeDialog}
            ...
            <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-3">
              <div className="grid gap-2 sm:grid-cols-2">
                <label className="space-y-1 text-xs text-zinc-400">
                  <span>参考项目标题</span>
                  <input
                    value={sourceProjectTitle}
                    onChange={(event) => { setSourceProjectTitle(event.target.value); resetResults(); }}
                    placeholder="例如：西施"
                ...
                <label className="space-y-1 text-xs text-zinc-400">
                  ...
                  <input
                    ...
                    onChange={(event) => { setTargetProjectTitle(event.target.value); resetResults(); }}
                    className="w-full rounded-lg border border-white/10 bg-white/[0.04] px-3 py-2 text-sm text-zinc-100 outline-none focus:border-sky-500/40"
                  />
                </label>
                <label className="space-y-1 text-xs text-zinc-400">
