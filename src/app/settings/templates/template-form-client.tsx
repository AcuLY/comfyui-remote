...
export function TemplateFormClient({
  ...
  // ── Prompt block management within a section ──
...
function SectionEditor({
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 space-y-3">
      {/* Header: drag, name, actions */}
      <div className="flex items-center gap-2">
        <div className="flex flex-col gap-0.5 text-zinc-600">
          <button
            onClick={() => onMove(-1)}
            disabled={isFirst}
            className="disabled:opacity-20 hover:text-zinc-400 transition"
          >
      ...
      {/* KSampler params (collapsed JSON editors) */}
      <KSamplerEditor
        label="KSampler 1"
        value={section.ksampler1}
        onChange={(v) => onUpdate({ ksampler1: v })}
      />
      <KSamplerEditor
        label="KSampler 2"
        value={section.ksampler2}
        onChange={(v) => onUpdate({ ksampler2: v })}
      />
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <span className="text-[10px] text-zinc-500">
            提示词块 ({(section.promptBlocks || []).length})
          </span>
          <button
            onClick={onAddBlock}
            className="text-[10px] text-zinc-500 hover:text-sky-400 transition"
          >
            + 添加
          </button>
        </div>
        {(section.promptBlocks || []).map((block, bi) => (
          <div key={bi} className="space-y-1 rounded-lg border border-white/5 bg-white/[0.02] p-2">
            <div className="flex items-center gap-2">
              <input
                type="text"
                value={block.label}
                ...
                placeholder="标签"
                className="w-24 shrink-0 rounded border border-white/10 bg-black/20 px-2 py-1 text-[11px] text-white outline-none placeholder:text-zinc-600"
              />
              <button
                onClick={() => onRemoveBlock(bi)}
                className="ml-auto text-zinc-600 hover:text-rose-400 transition"
              >
                <X className="size-3" />
              </button>
            </div>
            <textarea
              ...
              placeholder="正面提示词"
              rows={2}
              className="w-full resize-none rounded border border-white/10 bg-black/20 px-2 py-1 text-[11px] text-white outline-none placeholder:text-zinc-600"
            />
            <textarea
