...
- Phase 8 slice 91 moved section-detail result review state into `src/features/training/ui/use-project-section-results.ts`. The hook owns project-keyed result storage, active-section result filtering, and local review-status updates; the section detail page still owns review HTTP calls, API status mapping, toast feedback, and result grid wiring.
- Phase 8 slice 91 verification passed: red `node --import tsx --test tests/test-training-project-pages.test.ts tests/test-training-section-workflow.test.ts`, green `node --import tsx --test tests/test-training-project-pages.test.ts tests/test-training-section-workflow.test.ts`, `npx tsx scripts/docs/generate-repo-inventory.ts`, green `node --import tsx --test tests/test-training-project-pages.test.ts tests/test-training-section-workflow.test.ts tests/test-repo-inventory.test.ts`, `npm run lint`, and `npm test` with 1145 tests discovered, 1144 pass, 0 fail, 1 skipped.
- Phase 8 slice 92 moved project overview archive state into `src/features/training/ui/use-project-archive-state.ts`. The hook owns project-scoped local archived status and the archive-state setter; the overview page still owns archive/restore HTTP calls, route refresh, button pending state, toast feedback, and active-project header mapping.
- Phase 8 slice 92 verification passed: red `node --import tsx --test tests/test-training-project-pages.test.ts`, green `node --import tsx --test tests/test-training-project-pages.test.ts`, `npx tsx scripts/docs/generate-repo-inventory.ts`, green `node --import tsx --test tests/test-training-project-pages.test.ts tests/test-repo-inventory.test.ts`, `npm run lint`, and `npm test` with 1145 tests discovered, 1144 pass, 0 fail, 1 skipped.
- Phase 8 slice 93 moved create-project editable form state into `src/features/training/ui/use-project-create-form.ts`. The hook owns per-template default form values, template-context-scoped form state, and field update behavior; the create page still owns template selection side effects, seed sections, training defaults, reference selection/uploads, project creation HTTP calls, toasts, and navigation.
- Phase 8 slice 93 verification passed: red `node --import tsx --test tests/test-training-project-pages.test.ts`, green `node --import tsx --test tests/test-training-project-pages.test.ts`, `npx tsx scripts/docs/generate-repo-inventory.ts`, green `node --import tsx --test tests/test-training-project-pages.test.ts tests/test-repo-inventory.test.ts`, `npm run lint`, and `npm test` with 1145 tests discovered, 1144 pass, 0 fail, 1 skipped.
- Phase 8 slice 94 moved create-project training default state into `src/features/training/ui/use-project-create-training-defaults.ts`. The hook owns template-context-scoped `autoGenerateSamples` and `autoFreezeDataset` defaults plus update behavior; the create page still owns section seeds, reference selection/uploads, created project draft state, project creation HTTP calls, toasts, and navigation.
- Phase 8 slice 94 verification passed: red `node --import tsx --test tests/test-training-project-pages.test.ts`, green `node --import tsx --test tests/test-training-project-pages.test.ts`, `npx tsx scripts/docs/generate-repo-inventory.ts`, green `node --import tsx --test tests/test-training-project-pages.test.ts tests/test-repo-inventory.test.ts`, `npm run lint`, and `npm test` with 1145 tests discovered, 1144 pass, 0 fail, 1 skipped.
- Phase 8 slice 95 moved create-project created draft storage into `src/features/training/ui/use-project-created-draft.ts`. The hook owns the visible draft summary shape, template-context-scoped draft state, and draft update action; the create page still owns draft payload construction, section seed mutations, reference selection/uploads, project creation HTTP calls, toasts, and navigation.
- Phase 8 slice 95 verification passed: red `node --import tsx --test tests/test-training-project-pages.test.ts`, green `node --import tsx --test tests/test-training-project-pages.test.ts`, `npx tsx scripts/docs/generate-repo-inventory.ts`, green `node --import tsx --test tests/test-training-project-pages.test.ts tests/test-repo-inventory.test.ts`, `npm run lint`, and `npm test` with 1145 tests discovered, 1144 pass, 0 fail, 1 skipped.
- Phase 8 slice 96 moved create-project initial section seed storage into `src/features/training/ui/use-project-section-seeds.ts`. The hook owns template-context-scoped seed state, reset behavior, and the section seed update action; the create page still owns template selection side effects, copy/delete/toggle seed handlers, reference selection/uploads, draft payload construction, project creation HTTP calls, toasts, and navigation.
...
- Phase 8 slice 99 moved the shared project page shell into `src/features/training/ui/project-page-shell.tsx`. The component module owns the project tab config, `ProjectNav`, and `ProjectHeader`, so future overview/profile/sections/results/dataset/run page splits can import the shell without circular dependencies while `training-project-pages.tsx` keeps the remaining page implementations inline.
- Phase 8 slice 99 verification passed: red `node --import tsx --test tests/test-training-project-pages.test.ts`, green `node --import tsx --test tests/test-training-project-pages.test.ts`, `npx tsx scripts/docs/generate-repo-inventory.ts`, green `node --import tsx --test tests/test-training-project-pages.test.ts tests/test-training-section-workflow.test.ts tests/test-repo-inventory.test.ts`, `npm run lint`, and `npm test` with 1147 tests discovered, 1146 pass, 0 fail, 1 skipped.
- Phase 8 slice 100 moved the caption-aware training result grid into `src/features/training/ui/training-result-grid.tsx`. The focused component owns thumbnail cards, lightbox state, caption history entrypoints, and keep/reject callbacks while `training-project-pages.tsx` imports it for overview, section detail, results, dataset, and dataset revision pages.
...
- Phase 8 slice 103 moved the project profile page into `src/features/training/ui/training-project-profile-page.tsx`. The focused page owns reference image editing/upload/delete, add-to-results state, profile draft state, profile save API calls, profile text history loading, and text revision restore UI while `training-project-pages.tsx` keeps a compatibility re-export.
...
- Phase 8 slice 109 moved the project dataset revision page into `src/features/training/ui/training-project-dataset-revision-page.tsx`. The focused page owns explicit revision-id resolution, frozen sample caption snapshots, manifest rows, related training run lookup, and caption-aware result-grid wiring while `training-project-pages.tsx` keeps a compatibility re-export.
- Phase 8 slice 109 verification passed: red `node --import tsx --test tests/test-training-project-pages.test.ts tests/test-training-frontend-api-contract.test.ts`, green `node --import tsx --test tests/test-training-project-pages.test.ts tests/test-training-frontend-api-contract.test.ts`, `npx tsx scripts/docs/generate-repo-inventory.ts`, green `node --import tsx --test tests/test-training-project-pages.test.ts tests/test-training-frontend-api-contract.test.ts tests/test-training-image-thumbnail-contract.test.ts tests/test-training-list-layout.test.ts tests/test-repo-inventory.test.ts`, `npm run lint`, and `npm test` with 1155 tests discovered, 1154 pass, 0 fail, 1 skipped.
- Phase 8 slice 110 moved the project-scoped runs page into `src/features/training/ui/training-project-scoped-runs-page.tsx`. The focused page owns project/kind-scoped run interaction state, local delete/cancel/retry state, production delete/cancel/retry API calls, and route-header-only primary-action behavior while `training-project-pages.tsx` is now a compatibility re-export surface.
- Phase 8 slice 110 verification passed: red `node --import tsx --test tests/test-training-project-pages.test.ts tests/test-training-frontend-api-contract.test.ts`, green `node --import tsx --test tests/test-training-project-pages.test.ts tests/test-training-frontend-api-contract.test.ts`, `npx tsx scripts/docs/generate-repo-inventory.ts`, green `node --import tsx --test tests/test-training-project-pages.test.ts tests/test-training-frontend-api-contract.test.ts tests/test-training-image-thumbnail-contract.test.ts tests/test-training-list-layout.test.ts tests/test-repo-inventory.test.ts`, `npm run lint`, and `npm test` with 1156 tests discovered, 1155 pass, 0 fail, 1 skipped.
- Phase 8 slice 111 moved the resource-page URL search hook into `src/features/training/ui/use-resource-url-search.ts`. The hook owns the Next `useSearchParams` subscription for resource query hints while `training-resource-pages.tsx` consumes the focused hook and no longer imports `useSearchParams` directly.
- Phase 8 slice 111 verification passed: red `node --import tsx --test tests/test-training-resource-pages.test.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts`, `npx tsx scripts/docs/generate-repo-inventory.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts tests/test-training-frontend-api-contract.test.ts tests/test-training-list-layout.test.ts tests/test-repo-inventory.test.ts`, `npm run lint`, and `npm test` with 1156 tests discovered, 1155 pass, 0 fail, 1 skipped.
- Phase 8 slice 112 moved resource route/query helpers into `src/features/training/ui/training-resource-page-utils.ts`. The utility module owns preset/template route lookup, preset category/folder derivation, production training path detection, new-preset/new-template query hint parsing, and template-to-project href construction while `training-resource-pages.tsx` imports those focused helpers.
- Phase 8 slice 112 verification passed: red `node --import tsx --test tests/test-training-resource-pages.test.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts`, `npx tsx scripts/docs/generate-repo-inventory.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts tests/test-training-frontend-api-contract.test.ts tests/test-training-list-layout.test.ts tests/test-repo-inventory.test.ts`, `npm run lint`, and `npm test` with 1157 tests discovered, 1156 pass, 0 fail, 1 skipped.
- Phase 8 slice 113 moved training preset sort primitives into `src/features/training/ui/training-preset-sort-panel.tsx`. The focused module owns the sort item type, sort-order projection helper, sort panel, and sortable sort row while `training-resource-pages.tsx` keeps only the preset sort route state, persistence, and page composition.
- Phase 8 slice 113 verification passed: red `node --import tsx --test tests/test-training-resource-pages.test.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts`, `npx tsx scripts/docs/generate-repo-inventory.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts tests/test-training-frontend-api-contract.test.ts tests/test-training-list-layout.test.ts tests/test-repo-inventory.test.ts`, `npm run lint`, and `npm test` with 1158 tests discovered, 1157 pass, 0 fail, 1 skipped.
- Phase 8 slice 114 moved training preset library row primitives into `src/features/training/ui/training-preset-library-primitives.tsx`. The focused module owns preset status/usage labels, sortable preset item rows, and sortable category rail items while `training-resource-pages.tsx` keeps category/folder state, delete/reorder persistence, and page composition.
- Phase 8 slice 114 verification passed: red `node --import tsx --test tests/test-training-resource-pages.test.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts`, `npx tsx scripts/docs/generate-repo-inventory.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts tests/test-training-frontend-api-contract.test.ts tests/test-training-list-layout.test.ts tests/test-repo-inventory.test.ts`, `npm run lint`, and `npm test` with 1159 tests discovered, 1158 pass, 0 fail, 1 skipped.
- Phase 8 slice 115 moved training template list primitives into `src/features/training/ui/training-template-list-primitives.tsx`. The focused module owns template list status badges, template list anchor persistence, and sortable template list cards while `training-resource-pages.tsx` keeps template selection, delete/reorder persistence, scroll targeting, and page composition.
- Phase 8 slice 115 verification passed: red `node --import tsx --test tests/test-training-resource-pages.test.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts`, `npx tsx scripts/docs/generate-repo-inventory.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts tests/test-training-frontend-api-contract.test.ts tests/test-training-list-layout.test.ts tests/test-repo-inventory.test.ts`, `npm run lint`, and `npm test` with 1160 tests discovered, 1159 pass, 0 fail, 1 skipped.
- Phase 8 slice 116 moved the training template section row into `src/features/training/ui/training-template-section-row.tsx`. The focused module owns the shared template-section row type, drag handle wiring, edit/copy/delete actions, and unsaved-template route fallback while `training-resource-pages.tsx` keeps template section state, mutations, and page composition.
- Phase 8 slice 116 verification passed: red `node --import tsx --test tests/test-training-resource-pages.test.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts`, `npx tsx scripts/docs/generate-repo-inventory.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts tests/test-training-frontend-api-contract.test.ts tests/test-training-list-layout.test.ts tests/test-repo-inventory.test.ts`, `npm run lint`, and `npm test` with 1161 tests discovered, 1160 pass, 0 fail, 1 skipped.
- Phase 8 slice 117 moved training template page pure helpers into `src/features/training/ui/training-template-page-utils.ts`. The focused utility module owns template section state keys, scene-block move/ordinal helpers, template section copy/draft numbering, template/template-section ordering, and project-to-template-section seeding while `training-resource-pages.tsx` keeps only page state, mutation handlers, and composition.
- Phase 8 slice 117 verification passed: red `node --import tsx --test tests/test-training-resource-pages.test.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts`, `npx tsx scripts/docs/generate-repo-inventory.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts tests/test-training-template-section-workflow.test.ts tests/test-training-frontend-api-contract.test.ts tests/test-training-list-layout.test.ts tests/test-repo-inventory.test.ts`, `npm run lint`, and `npm test` with 1162 tests discovered, 1161 pass, 0 fail, 1 skipped.
- Phase 8 slice 118 moved the template scene block card into `src/features/training/ui/training-template-scene-block-card.tsx`. The focused component owns template block edit fields, move/delete actions, compact action rail, and the block patch type while `training-resource-pages.tsx` keeps template-section state, production block mutations, and composition.
- Phase 8 slice 118 verification passed: red `node --import tsx --test tests/test-training-resource-pages.test.ts tests/test-training-template-section-workflow.test.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts tests/test-training-template-section-workflow.test.ts`, `npx tsx scripts/docs/generate-repo-inventory.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts tests/test-training-template-section-workflow.test.ts tests/test-training-frontend-api-contract.test.ts tests/test-training-list-layout.test.ts tests/test-repo-inventory.test.ts`, `npm run lint`, and `npm test` with 1163 tests discovered, 1162 pass, 0 fail, 1 skipped.
- Phase 8 slice 119 moved the new-preset draft factory into `src/features/training/ui/training-resource-page-utils.ts`. The utility module now owns neutral new-preset defaults and source-artifact draft copy while `training-resource-pages.tsx` consumes the focused factory for preset creation routes.
- Phase 8 slice 119 verification passed: red `node --import tsx --test tests/test-training-resource-pages.test.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts tests/test-training-frontend-api-contract.test.ts tests/test-training-list-layout.test.ts tests/test-repo-inventory.test.ts`, `npm run lint`, and `npm test` with 1164 tests discovered, 1163 pass, 0 fail, 1 skipped.
- Phase 8 slice 120 moved training preset id-order projection into `src/features/training/ui/training-resource-page-utils.ts`. The helper now owns ordered-id projection plus missing-preset preservation while `training-resource-pages.tsx` keeps category/folder state, selection, delete, and persistence behavior.
- Phase 8 slice 120 verification passed: red `node --import tsx --test tests/test-training-resource-pages.test.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts tests/test-training-frontend-api-contract.test.ts tests/test-training-list-layout.test.ts tests/test-repo-inventory.test.ts`, `npm run lint`, and `npm test` with 1164 tests discovered, 1163 pass, 0 fail, 1 skipped.
- Phase 8 slice 121 moved training preset sort draft construction into `src/features/training/ui/training-preset-sort-panel.tsx`. The focused module now owns the visible sort draft type and the draft summary helper while `training-resource-pages.tsx` keeps route state, persistence, toasts, and page composition.
- Phase 8 slice 121 verification passed: red `node --import tsx --test tests/test-training-resource-pages.test.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts tests/test-training-frontend-api-contract.test.ts tests/test-training-list-layout.test.ts tests/test-repo-inventory.test.ts`, `npm run lint`, and `npm test` with 1164 tests discovered, 1163 pass, 0 fail, 1 skipped.
- Phase 8 slice 122 moved the training preset sort rules page into `src/features/training/ui/training-preset-sort-rules-page.tsx`. The focused page owns category/preset sort state, visible sort draft state, sort-rules API persistence, and sort-page composition while `training-resource-pages.tsx` keeps a compatibility re-export.
- Phase 8 slice 122 verification passed: red `node --import tsx --test tests/test-training-resource-pages.test.ts tests/test-training-frontend-api-contract.test.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts tests/test-training-frontend-api-contract.test.ts`, `npx tsx scripts/docs/generate-repo-inventory.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts tests/test-training-frontend-api-contract.test.ts tests/test-training-list-layout.test.ts tests/test-repo-inventory.test.ts`, `npm run lint`, and `npm test` with 1165 tests discovered, 1164 pass, 0 fail, 1 skipped.
- Phase 8 slice 123 moved the training templates list page into `src/features/training/ui/training-templates-page.tsx`. The focused page owns template selection, local hidden template state, list scroll targeting, template reorder/delete API persistence, and template-list composition while `training-resource-pages.tsx` keeps a compatibility re-export.
- Phase 8 slice 123 verification passed: red `node --import tsx --test tests/test-training-resource-pages.test.ts tests/test-training-frontend-api-contract.test.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts tests/test-training-frontend-api-contract.test.ts`, `npx tsx scripts/docs/generate-repo-inventory.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts tests/test-training-frontend-api-contract.test.ts tests/test-training-list-layout.test.ts tests/test-repo-inventory.test.ts`, `npm run lint`, and `npm test` with 1166 tests discovered, 1165 pass, 0 fail, 1 skipped.
- Phase 8 slice 124 moved the training template form page into `src/features/training/ui/training-template-form-page.tsx`. The focused page owns new/edit template form state, project-sourced template seeding, template save/delete API persistence, template-section add/copy/delete/reorder mutations, and editable template-section list composition while `training-resource-pages.tsx` keeps a compatibility re-export.
- Phase 8 slice 124 verification passed: red `node --import tsx --test tests/test-training-resource-pages.test.ts tests/test-training-frontend-api-contract.test.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts tests/test-training-frontend-api-contract.test.ts`, `npx tsx scripts/docs/generate-repo-inventory.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts tests/test-training-frontend-api-contract.test.ts tests/test-training-list-layout.test.ts tests/test-repo-inventory.test.ts`, `npm run lint`, and `npm test` with 1167 tests discovered, 1166 pass, 0 fail, 1 skipped.
- Phase 8 slice 125 moved the training template section page into `src/features/training/ui/training-template-section-page.tsx`. The focused page owns template-section form state, keyed scene-block state, preset/local block import and creation, block reorder/update/delete API persistence, section save draft/API persistence, and template-section composition while `training-resource-pages.tsx` keeps a compatibility re-export.
- Phase 8 slice 125 verification passed: red `node --import tsx --test tests/test-training-resource-pages.test.ts tests/test-training-frontend-api-contract.test.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts tests/test-training-frontend-api-contract.test.ts`, `npx tsx scripts/docs/generate-repo-inventory.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts tests/test-training-frontend-api-contract.test.ts tests/test-training-list-layout.test.ts tests/test-repo-inventory.test.ts`, `npm run lint`, stale workflow-source assertions fixed with green `node --import tsx --test tests/test-training-template-section-workflow.test.ts tests/test-training-resource-pages.test.ts tests/test-training-frontend-api-contract.test.ts`, and `npm test` with 1168 tests discovered, 1167 pass, 0 fail, 1 skipped.
- Phase 8 slice 126 moved the training presets library page into `src/features/training/ui/training-presets-page.tsx`. The focused page owns category/folder selection, preset selection state, local delete hiding, production delete API calls, preset/category reorder state, sort-rule API persistence, and preset-library composition while `training-resource-pages.tsx` keeps a compatibility re-export plus the preset detail editor.
- Phase 8 slice 126 verification passed: red `node --import tsx --test tests/test-training-resource-pages.test.ts tests/test-training-frontend-api-contract.test.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts tests/test-training-frontend-api-contract.test.ts`, `npx tsx scripts/docs/generate-repo-inventory.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts tests/test-training-frontend-api-contract.test.ts tests/test-training-list-layout.test.ts tests/test-repo-inventory.test.ts`, `npm run lint`, and `npm test` with 1169 tests discovered, 1168 pass, 0 fail, 1 skipped.
- Phase 8 slice 127 moved the training preset detail editor into `src/features/training/ui/training-preset-detail-page.tsx`. The focused page owns new/edit preset context state, source-artifact query hints, preset save/create/delete API persistence, visible draft state, usage-impact display, and editor composition while `training-resource-pages.tsx` is now only the compatibility re-export surface for resource pages.
- Phase 8 slice 127 verification passed: red `node --import tsx --test tests/test-training-resource-pages.test.ts tests/test-training-frontend-api-contract.test.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts tests/test-training-frontend-api-contract.test.ts`, `npx tsx scripts/docs/generate-repo-inventory.ts`, green `node --import tsx --test tests/test-training-resource-pages.test.ts tests/test-training-frontend-api-contract.test.ts tests/test-training-list-layout.test.ts tests/test-repo-inventory.test.ts`, `npm run lint`, and `npm test` with 1170 tests discovered, 1169 pass, 0 fail, 1 skipped.
- Phase 8 slice 128 moved the run-detail pure helper layer into `src/features/training/ui/training-run-detail-utils.ts`. The focused utility owns production-route detection, run lookup, dataset-version detail titles, progress/config text, generation-output result projection, and training artifact/preset lifecycle labels while `training-run-detail-page.tsx` keeps JSX state, actions, lightboxes, and API mutation handlers.
...
- Phase 11 slice 141 extracted `src/server/worker/training/heartbeat.ts` and `src/server/worker/training/task-json.ts` from `src/server/worker/training/task-api.ts` so heartbeat request validation, progress writes, and Prisma-compatible JSON normalization have dedicated boundaries while `task-api.ts` keeps compatibility re-exports for existing routes.
- Phase 11 slice 141 verification passed: red `node --import tsx --test tests/test-worker-boundary-governance.test.ts`, green `node --import tsx --test tests/test-worker-boundary-governance.test.ts`, targeted heartbeat route/entrypoint regression with `node --import tsx --test --test-name-pattern "GET /api/training full workflow heartbeat steps reuse the worker task HTTP contract|managed scheduler and worker endpoints can advance generation and training runs through completion|managed worker endpoints can mark generation and training runs as failed through /api/training" tests/test-training-api-routes.test.ts` and `node --import tsx --test tests/test-training-worker-entrypoints.test.ts`, `npx tsx scripts/docs/generate-repo-inventory.ts`, green `node --import tsx --test tests/test-worker-boundary-governance.test.ts tests/test-training-api-boundary.test.ts tests/test-training-api-routes.test.ts tests/test-training-worker-entrypoints.test.ts tests/test-repo-inventory.test.ts`, `npm run lint`, and `npm test` with 1194 tests discovered, 1193 pass, 0 fail, 1 skipped.
- Phase 11 slice 142 extracted `src/server/worker/training/completion.ts` from `src/server/worker/training/task-api.ts` so complete request validation, generation output persistence, final LoRA/log artifact registration, dataset-ready transitions, and target-specific completion wrappers have a dedicated boundary while `task-api.ts` keeps compatibility re-exports for existing routes.
- Phase 11 slice 142 verification passed: red `node --import tsx --test tests/test-worker-boundary-governance.test.ts`, green `node --import tsx --test tests/test-worker-boundary-governance.test.ts`, targeted completion route/entrypoint regression with `node --import tsx --test --test-name-pattern "GET /api/training worker task resources declare request and response contracts|GET /api/training domain worker callback resources declare request and response contracts|managed scheduler and worker endpoints can advance generation and training runs through completion" tests/test-training-api-routes.test.ts` and `node --import tsx --test --test-name-pattern "training worker dry-run mock-complete uses Training task lifecycle routes|training worker local_wsl_sd_scripts adapter completes a configured real runner|training worker supervisor targets Training-named worker task HTTP routes" tests/test-training-worker-entrypoints.test.ts`, `npx tsx scripts/docs/generate-repo-inventory.ts`, green `node --import tsx --test tests/test-worker-boundary-governance.test.ts tests/test-training-api-boundary.test.ts tests/test-training-api-routes.test.ts tests/test-training-worker-entrypoints.test.ts tests/test-repo-inventory.test.ts`, `npm run lint`, and `npm test` with 1195 tests discovered, 1194 pass, 0 fail, 1 skipped.
- Phase 11 slice 143 extracted `src/server/worker/training/failure.ts` from `src/server/worker/training/task-api.ts` so fail request validation, generation/section run error writes, dataset and training run failed transitions, provider error payload serialization, and target-specific fail wrappers have a dedicated boundary while `task-api.ts` keeps compatibility re-exports for existing routes.
- Phase 11 slice 143 verification passed: red `node --import tsx --test tests/test-worker-boundary-governance.test.ts`, green `node --import tsx --test tests/test-worker-boundary-governance.test.ts`, targeted failure route/entrypoint regression with `node --import tsx --test --test-name-pattern "GET /api/training worker task resources declare request and response contracts|GET /api/training domain worker callback resources declare request and response contracts|managed worker endpoints can mark generation and training runs as failed through /api/training" tests/test-training-api-routes.test.ts` and `node --import tsx --test --test-name-pattern "training worker reports local_wsl_sd_scripts readiness errors when runner config is missing|training worker supervisor targets Training-named worker task HTTP routes" tests/test-training-worker-entrypoints.test.ts`, `npx tsx scripts/docs/generate-repo-inventory.ts`, green `node --import tsx --test tests/test-worker-boundary-governance.test.ts tests/test-training-api-boundary.test.ts tests/test-training-api-routes.test.ts tests/test-training-worker-entrypoints.test.ts tests/test-repo-inventory.test.ts`, `npm run lint`, and `npm test` with 1196 tests discovered, 1195 pass, 0 fail, 1 skipped.
- Phase 11 slice 144 extracted `src/server/worker/training/scheduler.ts` from `src/server/worker/training/task-api.ts` so queue status, scheduler tick target routing, scheduler leasing, and training-run progress writes have a dedicated boundary while `task-api.ts` is now a pure compatibility barrel for the split worker API.
- Phase 11 slice 144 verification passed: red `node --import tsx --test tests/test-worker-boundary-governance.test.ts`, green `node --import tsx --test tests/test-worker-boundary-governance.test.ts`, targeted scheduler route/status/progress regression with `node --import tsx --test --test-name-pattern "GET /api/training manifest exposes scheduler and worker operations for agent-driven execution|GET /api/training full workflow binds scheduler ticks to the target run handoff|managed scheduler tick can advance a target queued generation run|managed scheduler and worker endpoints can advance generation and training runs through completion|GET /api/training/scheduler/status exposes a training scheduler snapshot|GET /api/training/worker/status exposes production HTTP-only run queue counts" tests/test-training-api-routes.test.ts`, `npx tsx scripts/docs/generate-repo-inventory.ts`, green `node --import tsx --test tests/test-worker-boundary-governance.test.ts tests/test-training-api-boundary.test.ts tests/test-training-api-routes.test.ts tests/test-training-worker-entrypoints.test.ts tests/test-repo-inventory.test.ts`, `npm run lint`, and `npm test` with 1197 tests discovered, 1196 pass, 0 fail, 1 skipped.
- Phase 11 slice 145 extracted `scripts/training/training-worker-runtime.ts` from `scripts/training/training-worker.ts` so the training launcher CLI stays a thin entrypoint while dry-run/mock completion, local_wsl_sd_scripts adapter execution, runner output parsing, and mock artifact shaping are importable runtime behavior.
...
- Phase 11 slice 148 tightened Training worker lifecycle schemas so lease owners, heartbeat progress payloads, failure summaries, and provider error payloads stay validated at the shared `src/lib/training/schemas.ts` boundary. Provider error backend messages are now trimmed/nonblank and provider error payloads reject unknown fields instead of silently stripping them; `docs/worker-boundaries.md` records the no-ad-hoc-validation rule.
...
- [x] Split `training-project-pages.tsx` by page:
  ...
  - [x] Dataset page.
  - [x] Dataset revision page.
  - [x] Project-scoped runs page.
...
- `src/server/worker/training/failure.ts`
...
- [x] Ensure heartbeat, lease ownership, failure summary, and provider error payloads are schema-validated.
- [x] Keep ComfyUI cancellation semantics strict: HTTP failures must fail current batch and stop later batches.
