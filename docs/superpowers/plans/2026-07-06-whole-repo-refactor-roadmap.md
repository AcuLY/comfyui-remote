...
| Project results | `src/app/projects/[projectId]/results/page.tsx` owns aggregate project-results loading and `notFound()`. | `src/app/projects/[projectId]/results/project-results-client.tsx` currently owns sidebar navigation, filters, collapsed rows, gallery/lightbox state, quick censor, and review mutations with `src/lib/review-lightbox-state.ts`. | Split route container, toolbar/filter state, gallery/list component, lightbox adapter, and mutation adapter. |
