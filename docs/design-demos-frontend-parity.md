...
- Phase 0/1 主功能同步质量检查与 auto-review：当前只记录为后端/CLI 流程，不在 `/design-demos` 增加可见 UI。
...
- Phase 3 如果补齐 auto-review 可视化，应覆盖 `auto_trash`、`candidate`、`review` 在审核图、运行分组和小节分组里的状态关系；当前阶段不增加对应 UI。
