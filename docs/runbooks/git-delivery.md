...
本流程已于 2026-07-14 在当前文档治理分支上完成真实演练：先检查完整工作树，只暂存获准路径，审查暂存差异并运行暂存检查，再提交、推送和复核上游；5 个无关个人脚本始终保持未跟踪、未修改且未暂存。当前 `verifiedBy` 继续核对仓库 Git 边界、忽略规则和文档合同。暂存范围有歧义、误暂存禁止文件、提交失败和推送失败等故障恢复分支尚未实际演练。
