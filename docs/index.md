...
| --- | --- | --- | --- | --- | --- |
| architecture | `docs/documentation-map.md`, `docs/architecture/README.md`, `docs/worker-boundaries.md`, `docs/prisma-provider-matrix.md`, `docs/prisma-schema-compatibility.md`, `docs/analysis/**` | architecture reference plus current map | documentation-system | module boundaries, dependency rules, data flow, or refactor scope change | documentation governance tests and inventory check |
...
| UI design | `DESIGN.md`, `docs/frontend-design-guide.md`, `docs/design-demos-frontend-parity.md`, `docs/ui/component-boundaries.md`, `docs/ui/design-demo-governance.md`, `docs/ui/legacy-static-design-demos.md`, `docs/ui/project-page-boundaries.md` | product/design reference | ui-design-system | shared shell, page pattern, token, accessibility, design-demo, or project page boundary changes | UI/source-contract tests |
| training | `docs/prototypes/README.md`, `src/app/training/[[...route]]/page.tsx`, `src/features/training`, `src/server/services/training`, `src/server/repositories/training` | prototype intent plus current source | training | training route, API, service, repository, or prototype changes | training frontend/API/service tests |
| testing | `docs/testing/README.md`, `tests/README.md`, `tests/fixtures/**` | testing reference | test-infrastructure | test grouping, fixture builders, DB bootstrap, native module setup, or verification matrix changes | targeted test plus `npm test` |
| queue/worker behavior | `agent-rules/deploy/queue.md`, `docs/worker-boundaries.md`, `src/server/worker`, `src/lib/actions/run-lifecycle.ts` | runbook plus current source | queue-worker | queue pause/resume, worker lease, active run, or scheduler changes | queue, run, worker, and deploy tests |
...
| prototype | Intent or visual reference, not runtime truth. | `docs/prototypes/**`, archived static demos under `docs/archive/design-demos/**` |
| historical record | Completed handoff, progress, PRD, or older plan retained for context. | `docs/archive/historical/**`, `docs/archive/plans/**`, `docs/archive/prd/**`, `docs/archive/superpowers/plans/**` |
...
- Keep old plans and PRDs in `docs/archive/**` unless their durable decisions are extracted into current docs.
