...
- `prisma/` — 数据库 schema + migration（17 模型 + 5 枚举）
- `config/` — 配置文件（`path-maps.json` + `workflows/*.json`）
- `src/app/` — 23 个页面 + 84 个 API 路由
...
### Agent API
- `GET /api/agent/projects` — Project 列表（搜索/状态筛选）
- `GET /api/agent/projects/:id/context` — Project 完整上下文
- `POST /api/agent/projects/:id/update` — AI 修改 Project 参数
- `POST /api/agent/projects/:id/run-all` — AI 触发所有 enabled Section 运行
- `POST /api/agent/sections/:id/run` — AI 触发运行
...
- Prisma schema 覆盖：PresetCategory、Preset、PresetVariant、PresetGroup、Project（含 presetBindings JSON 字段）、ProjectSection、PromptBlock（含 bindingId）、Run、ImageResult、TrashRecord、LoraAsset（含 triggerWords）、ProjectRevision、AuditLog
- 有 seed 脚本填充测试数据
...
| `/projects` | Prisma | ✅ 创建/编辑/复制/运行 |
| `/projects/new` | Prisma | ✅ 创建表单 |
| `/projects/[projectId]` | Prisma | ✅ 运行整组/复制/运行单节/编辑 |
| `/projects/[projectId]/edit` | Prisma | ✅ 参数编辑表单 |
| `/trash` | Prisma | ✅ 恢复按钮 |
| `/assets/models` | Prisma | ✅ 模型文件管理 |
...
- `runProject` / `runSection` — 调用 repository 层入队
- `createProject` / `updateProject` / `copyProject` — 调用 repository 层
- `uploadLora` — LoRA 上传（文件写入 + DB 登记）
- PresetCategory / Preset / PresetVariant / PresetGroup CRUD

### REST API Service 层（`src/server/services/`）
- `project-service.ts` — Project 完整 CRUD + 入队 + 复制 + 严格输入校验
...
- `revision-service.ts` — Project 更新前自动快照（ProjectRevision 表），支持修订历史查看
...
- 前端 Section 模板设置页可选择关联 workflow 模板
...
### AuditLog + ProjectRevision
- `AuditLog` 表覆盖：Project CRUD、enqueue、review keep/trash/restore、Worker claim/done/failed
- Agent API 路由传入 `ActorType.agent`，可区分 AI 和人工操作
- `ProjectRevision` 表在 Project/Section 更新前自动快照，支持回溯"这一轮参数怎么改过"
- API 端点：`GET /api/projects/:id/revisions`、`GET /api/projects/:id/revisions/:revisionNumber`、`GET /api/audit-logs`
...
- 11 个 Tools：list_projects / update_project / update_project_section / run_all_sections / run_section / review_images / list_prompt_blocks / add_prompt_block / update_prompt_block / remove_prompt_block / reorder_prompt_blocks
- 6 个 Resources：project context / run context / workflow templates / workflow detail / project revisions / revision snapshot
