...
- `keepImages` / `trashImages` — 批量 keep/trash（DB 更新，文件移动待接入）
- `restoreImage` — 回收站恢复（DB 更新，文件移动待接入）
- `runJob` / `runPosition` — 运行整组/单节
- `createJob` / `updateJob` / `copyJob` — Job CRUD
...
### 尚未实现
- Server Actions 中的文件移动（trash/restore 只更新 DB，不移动文件）
- 自定义 workflow 模板系统
- 本机完整链路验证文档
