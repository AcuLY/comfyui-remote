...
`src/server/worker/training/completion.ts` owns training worker completion handling. It validates complete requests, persists generation outputs, registers final LoRA and log artifacts, marks dataset revisions ready, and exposes target-specific completion wrappers for generation tasks and training runs.

`src/server/worker/training/failure.ts` owns training worker failure handling. It validates fail requests, records generation task and section run errors, marks dataset revisions or training runs failed, and preserves provider error payloads in the shared serialized worker task shape.

`src/server/worker/training/scheduler.ts` owns training worker scheduler handling. It reports queue status, advances target-scoped or worker-type scheduler ticks through the leasing boundary, and persists training run scheduler progress updates.
