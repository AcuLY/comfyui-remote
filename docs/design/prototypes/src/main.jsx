...
const colorSpecs = [
  ...
  ['image', '图像生产', '绿色模块强调'], ['training', 'LoRA 训练', '玫瑰粉模块强调'],
];
...
function App() {
  ...
  const [currentSection, setCurrentSection] = useState(() => sections.some(([id]) => id === location.hash.slice(1)) ? location.hash.slice(1) : 'colors');
  ...
  useEffect(() => {
    ...
    const updateSection = () => {
      frame = 0;
      const height = chrome.current?.getBoundingClientRect().height || 0;
      document.documentElement.style.setProperty('--foundation-chrome-height', `${height}px`);
      let active = 'colors';
      for (const [id] of sections) {
        if (document.getElementById(id)?.getBoundingClientRect().top <= height + 32) active = id;
      }
      if (window.scrollY + window.innerHeight >= document.documentElement.scrollHeight - 2) active = sections.at(-1)[0];
      setCurrentSection((previous) => previous === active ? previous : active);
    };
    ...
    schedule();
    return () => { observer.disconnect(); cancelAnimationFrame(frame); window.removeEventListener('scroll', schedule); window.removeEventListener('resize', schedule); document.documentElement.style.removeProperty('--foundation-chrome-height'); };
  }, []);
  useEffect(() => {
  ...
  function submitPreview(event) {
    ...
    if (!previewName.trim()) { setPreviewError('请输入名称，再试一次。'); requestAnimationFrame(() => document.getElementById('preview-name')?.focus()); return; }
  ...
  function goToSection(id) {
    ...
    requestAnimationFrame(() => {
      if (location.hash === `#${id}`) document.getElementById(id)?.scrollIntoView({ block: 'start', behavior: 'auto' });
    });
  }
  ...
  return <PrimeReactProvider value={{ ripple: false, locale: 'zh-CN', hideOverlaysOnDocumentScrolling: true }}><div className="foundation-page">
    ...
    <div className="workspace">
      ...
      <main id="foundation-main" className="foundation-main" tabIndex={-1}>
        <div className="page-heading"><div><h1>基础设计</h1><p>可操作的组件、尺寸与间距样本。</p></div><a className="text-link" href="#dimensions">查看适配规则<i className="pi pi-arrow-down" aria-hidden="true" /></a></div>
        ...
        <Section id="colors" title="色彩与主题" description="中性色承托内容，模块色标识操作；成功、失败等状态使用各自的语义色。">
          <div className="color-layout">
            ...
            <form className="live-preview" onSubmit={submitPreview}>
              ...
              <Field id="preview-name" label="名称" error={previewError} liveError><InputText id="preview-name" value={previewName} onChange={(e) => { setPreviewName(e.target.value); setSavedPreview(false); }} invalid={Boolean(previewError)} aria-invalid={Boolean(previewError)} aria-describedby={previewError ? 'preview-name-error' : undefined} /></Field>
        ...
        <Section id="dimensions" title="尺寸与适配" description="标准密度固定。控件按设备适配，同一行保持等高，触摸范围单独保障。">
        ...
        <Section id="feedback" title="状态与反馈" description="状态带文字和图标。加载保持布局，错误说明原因并提供恢复动作。">
          ...
          <div className="state-workbench"><div className="state-toolbar"><h3>内容状态样本</h3><SelectButton value={contentState} options={['内容', '加载', '空内容', '失败']} onChange={(e) => { if (e.value) setContentState(e.value); }} allowEmpty={false} aria-label="内容状态" /></div>
            <div className="state-stage" aria-live="polite">
              ...
              {contentState === '加载' ? <div className="skeleton-row" aria-label="正在加载样本" aria-busy="true"><Skeleton width="var(--foundation-leading-size)" height="var(--foundation-leading-size)" borderRadius="8px" /><div><Skeleton width="60%" height="16px" /><Skeleton width="90%" height="12px" /></div><Skeleton width="64px" height="24px" /></div> : null}
