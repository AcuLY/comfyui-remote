...
const treeNodes = (parent = null) => folders.filter(f => f.parent === parent).map(f => ({ key: f.key, label: f.label, icon: 'pi pi-folder', children: treeNodes(f.key) }));
const nodes = treeNodes();
const expandedDefault = { root: true, portrait: true, studio: true, scene: true };
const initialGroups = {
...
function App() {
  ...
  const [folder, setFolder] = useState('portrait');
  ...
  function chooseFolder(key) {
    if (!key) return;
    setFolder(key); setSelected([]); setQuery(''); setFoldersOpen(false);
  }
  ...
  function resetSample() { setGroups(initialGroups); setFolder('portrait'); setExpanded(expandedDefault); setTargetExpanded(expandedDefault); changeState('content'); toast.current?.clear(); }
  ...
  function applyOrder() {
    ...
    toast.current.show({ severity: 'success', summary: '顺序已更新', detail: `已应用到「${folderName(folder)}」的示例内容。`, life: 3000 });
  }
  ...
  const folderTree = <Tree className="org-tree" value={nodes} selectionMode="single" selectionKeys={folder} onSelectionChange={e => chooseFolder(e.value)} expandedKeys={expanded} onToggle={e => setExpanded(e.value)} ariaLabel="文件夹目录" nodeTemplate={node => <span className="org-folder-name">{node.label}</span>} />;
  ...
  return <div className="list-page org-page">
        ...
        <div className="org-panel">
          ...
          <div className="org-content" aria-busy={previewState === 'loading'}>
            {previewState === 'loading' ? <LoadingRows /> : previewState === 'error' ? <div className="org-empty" role="alert"><i className="pi pi-exclamation-circle" aria-hidden="true" /><h3>内容加载失败</h3><p>文件夹位置已保留，可以重试。</p><Button label="重试" icon="pi pi-refresh" onClick={retry} /></div>
              : !items.length ? <div className="org-empty"><i className="pi pi-folder-open" aria-hidden="true" /><h3>这个文件夹还是空的</h3><p>可以从其他文件夹选择内容，再移动到这里。</p><Button outlined label="返回人像样本" onClick={() => { chooseFolder('portrait'); changeState('content'); }} /></div>
                : <ListBox className="org-content-list" options={items} value={selected} onChange={e => { if (editable) setSelected(e.value || []); }} optionLabel="name" dataKey="id" multiple metaKeySelection={false} pt={{ list: { 'aria-readonly': !editable }, filterContainer: { style: { maxWidth: '400px' } } }} itemTemplate={recordTemplate} aria-label="当前文件夹内容" listStyle={{ maxHeight: 'none' }} filter filterBy="name,note" filterValue={query} onFilterValueChange={e => { setQuery(e.value); setSelected([]); }} filterPlaceholder="搜索当前文件夹" filterInputProps={{ 'aria-label': '搜索当前文件夹' }} emptyFilterMessage="没有匹配的内容，试试其他关键词。" />}
          </div>
          {query && <p className="org-filter-report" aria-live="polite">当前匹配 {visibleItems.length} 项；排序作用于文件夹全部内容。</p>}
        </div>
      </section>
      <div className="list-review-notes"><p><i className="pi pi-info-circle" aria-hidden="true" />排序只改变当前文件夹内的顺序；移动后追加到目标末尾。</p><p>固定模拟数据，刷新恢复初始内容。<a href="../../ui-design-roadmap.md">查看设计路线</a></p></div>
      <footer className="page-footer"><span>R01-02 · 组件组合待审核</span><span>{theme === 'dark' ? '深色' : '浅色'} · {preference.module === 'training' ? '训练' : '生产'}</span></footer>
    </main>
    <Dialog header="选择文件夹" visible={foldersOpen} onHide={() => setFoldersOpen(false)} className="org-folder-dialog" draggable={false} blockScroll>{folderTree}</Dialog>
    ...
    <Dialog header="预览设置" visible={settingsOpen} onHide={() => setSettingsOpen(false)} className="list-preview-dialog" draggable={false} blockScroll footer={<Button label="完成" onClick={() => setSettingsOpen(false)} />}>
      <div className="list-settings-fields"><div><span id="org-module">模块色</span><SelectButton value={preference.module} options={moduleOptions} onChange={e => updatePreference('module', e.value)} aria-labelledby="org-module" allowEmpty={false} /></div><div><span id="org-theme">主题</span><SelectButton value={preference.theme} options={themeOptions} onChange={e => updatePreference('theme', e.value)} aria-labelledby="org-theme" allowEmpty={false} /></div><div><label htmlFor="org-state">预览状态</label><Dropdown inputId="org-state" value={previewState} options={stateOptions} onChange={e => changeState(e.value)} /></div><div className="org-failure-option"><Checkbox inputId="org-move-failure" checked={moveFailure} onChange={e => setMoveFailure(e.checked)} /><label htmlFor="org-move-failure">模拟下一次移动失败</label></div><Button outlined label="重置全部示例" icon="pi pi-refresh" onClick={resetSample} /></div>
    </Dialog>
  </div>;
}
