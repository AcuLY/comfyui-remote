...
| N-02 | 两模块都按任务、项目、预制、模板显示四入口；模型、监控与日志、设置为全局工具 | 桌面稳定收放的侧栏及主题快捷切换；手机模块、四入口与更多共同位于底部，顶部留页面操作。2026-09-10用户指令覆盖原 `IA-06` 顶部模块布局；注销仍按 `IA-05/06` 保留模拟入口，详见[R02-01修订](reviews/R02-01.md#2026-09-10-用户反馈与修订规则) |
