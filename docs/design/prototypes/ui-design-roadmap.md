...
## 当前状态：已知问题修复，R02 与统一主题修订待审核

用户于2026-09-12要求继续既有 [R02-02 项目上下文与导航恢复](reviews/R02-02.md)，2026-09-13授权“先处理已知的问题”。本轮先修订契约，再修复已呈现的归档只读、独立资源及分类型任务关系、唯一身份、来源恢复、根目录／全部范围与临界布局；在本原型补齐项目／小节多选移动、文件夹排序和小节拖排。共享主题恢复、键盘焦点、导航反馈及状态记录同时修正，复盘 `I01`～`I14` 的实际验收见任务记录。

`R01-01` 的 `30fd53db` 确认历史保留，`R01-02` 保持暂停；基础方向及 `8029fdfb` 历史批准不扩大为统一主题全部修订获批。基础统一主题修订及 `R02-01/02` 均待用户审核。当前组织交互修复不等于 `R05-02`／`R05-05` 全任务或生命周期获批，删除和正式执行仍在后续任务，其余77项仍为拆解草案。
...
三个分册事前登记79项任务（共享26、生产30、训练23），其中 `R02-01`、`R02-02` 已分别选定制作，其余77项仍待确认；另保留上方2项历史台账。任务数量不代表页面数量或制作完成。
...
| --- | --- | --- | --- |
| R00 | 基础规则 F-01～F-14 | [基础规范](design-foundations.md)、[基础台账](foundations/README.md) | 既有方向及批准历史保留；统一主题修订待用户审核 |
| R01 | 已有通用样本记录 | 上方R01-01／R01-02独立台账 | 一项已确认、一项暂停；不再作为全部页面的门禁 |
| R02（R02-01～R02-03，3项） | 导航与登录 | [共享分册](ui-design-shared-plan.md#r02-导航与登录) | `R02-01/02` 已知问题修订待用户审核；`R02-03` 仍为草案 |
...
| R05（R05-01～R05-06，6项） | 生产项目与小节组织 | [生产分册](ui-design-production-plan.md) | 全任务仍为草案；`R02-02` 修复部分已呈现组织能力，不代表 `R05-02/05` 完成或获批 |
...
2. “继续下一个”只能对应**已登记、范围明确、并在已确认顺序中唯一确定**的下一项。2026-09-12用户明确继续下一项，当前执行队列仅为既有R02-02，其范围见对应记录；不顺带启动R02-03或批准其他批次顺序。如果出现多个候选或未解决的业务范围，先澄清，不能临时取号。
...
现有原型和审核文件保留为相应版本的证据。`R01-02` 中的交互只表示暂停的历史实验；本轮纠正入口与页尾状态文案，不继续其交互设计，也不将其转为正式模型管理页。明确标为历史的44px普通触控测量保留，当前普通触控控件40px、图标按钮／关闭／分页44px的规则不被旧测量覆盖。
...
| Q-02 拖放、缩放的库能力 | 各业务任务核对所用v10公开能力；`R02-02` 已核对树节点、表格行与独立列表拖放边界 | 当前图片网格跨文件夹多选拖放由应用层原生 `HTML` 桌面拖放承担，移动菜单及上移／下移提供替代；其他拖放／缩放按具体任务判断，不作为全页面前置 |
...
| N-06 | 稳定深链、浏览器前进／后退、刷新恢复、有效页面位置、查询／展开／页签／滚动恢复（点击模块仍进入对应任务） | 抽屉保留来源列表；可分享筛选进入 URL；图片多选只属于当前 UI 会话，不变成持久业务状态 |
...
本轮已按 [R02-02 已知问题验收表](reviews/R02-02.md#本轮验收已知问题修复)记录修复及针对性验证；逐项区分浏览器交互、模型测试与源码核对，真机触屏和完整状态矩阵未验证。最终合并门禁由收尾记录补充，设计仍交用户审核。旧首稿与首轮图片／文件夹验证保留为历史；登录、完整业务编辑、删除和正式执行不在本轮，`R01-02` 保持暂停，不自动进入下一项。
