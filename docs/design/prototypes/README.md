...
2026-09-14 当前工作为[完整生产模块重做](reviews/production-rebuild.md)：用户明确要求放弃现有生产项目/任务方案，任务和项目以设计演示为主要参考，预制和模板以旧前端为主要参考。入口为[生产工作台](http://127.0.0.1:5178/shell/project-context/#production/tasks)，覆盖任务审核、项目与小节、预制和模板；本页下方先前审核状态作为历史保留。当前实现与验证由重做记录维护，不表示用户已批准新版。
...
用户已确认 [R01-01 列表组合](reviews/R01.md)的 `30fd53db` 版本，`R01-02` 保持暂停。`R02-01` 与基础统一主题修订的审核状态不变；`R02-02` 生产项目和任务方案已被用户要求替换，旧记录仅供回查。当前生产模块由[重做记录](reviews/production-rebuild.md)维护范围和验证，训练及全局工具继续按各自任务推进。
...
| P-04 | 业务组合 | 筛选工具栏、任务行、图片卡片、参数表单等 | `R01-01` 确认历史保留（`30fd53db`）；共享修订待审核；`R01-02` 定位偏差，暂停 |
| P-05 | 完整页面 | 两模块的项目、任务、预制、模板及全局工具页面 | 生产任务、项目、预制和模板已授权合并重做；当前产物待用户审核，训练及全局工具未扩大范围 |
...
当前审核入口为[生产工作台](http://127.0.0.1:5178/shell/project-context/#production/tasks)，从侧栏或底栏进入项目、预制和模板；范围、来源及实测查阅[重做记录](reviews/production-rebuild.md)。[R02-01独立外壳](http://127.0.0.1:5178/shell/navigation/)继续保留；[R01-02历史原型](http://127.0.0.1:5178/components/organization/)保持暂停。[列表组合](http://127.0.0.1:5178/components/lists/)的已确认样本保留，共享主题修订另行验证和审核。

生产原型包含项目／小节组织、编辑、任务控制、图片审核、打码、导出、预制和模板的本地操作。归档永久只读；演示执行与数据保存在当前浏览器会话，可从预览设置重置。它不调用真实生产服务，也不把原型操作当成真实生成或服务器交付。
...
| `shell/navigation/index.html` | `R02-01` 两模块与全局工具导航原型入口，业务内容仅占位 |
| [reviews/R02-01.md](reviews/R02-01.md) | 导航外壳的事前范围、适用验收和设计审核状态 |
...
按[总路线](ui-design-roadmap.md)先登记并明确具体任务。`R01-01` 已确认，`R01-02` 暂停且不作为其他页面前置；当前仅 `R02-01` 已选定制作，其他新拆解条目仍为待确认草案。已批准任务中的局部修正仍绑定原编号；新增目的、对象或流程必须事前变更任务说明，不先写原型再补文档。
