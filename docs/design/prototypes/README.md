...
用户已确认 [R01-01 列表组合](reviews/R01.md)的 `30fd53db` 版本，`R01-02` 保持暂停。2026-09-13用户授权“先处理已知的问题”，本轮修复 [R02-02 项目上下文与导航恢复](reviews/R02-02.md)的业务关系、只读、组织交互和恢复，并同步基础／列表主题集成、导航反馈及状态记录。`R02-01/02` 与基础统一主题修订均待用户审核；历史确认不由修复自动扩大，其余77项仍为草案。
...
| P-05 | 完整页面 | 两模块的项目、任务、预制、模板及全局工具页面 | `R02-01/02` 待用户审核；当前修复关联明确的本地导航与组织样本，完整业务页面未开始 |
...
当前审核入口为 [R02-02 项目上下文](http://127.0.0.1:5178/shell/project-context/#production/projects)，契约与复盘 `I01`～`I14` 的本轮实测查阅[独立记录](reviews/R02-02.md)；[R02-01独立外壳](http://127.0.0.1:5178/shell/navigation/)继续保留。[R01-02历史原型](http://127.0.0.1:5178/components/organization/)明确标为暂停，供回查。[列表组合](http://127.0.0.1:5178/components/lists/)的已确认样本保留，共享主题修订另行验证和审核。

本轮在 `R02-02` 补齐单项／多选项目移动、文件夹排序、小节拖排及跨文件夹移动，并提供键盘／触屏等价入口；这不等于 `R05-02`／`R05-05` 全任务或生命周期获批。归档项目在已呈现入口中持续只读，删除、归档执行与生成／训练正式执行不在本轮原型修复范围。
