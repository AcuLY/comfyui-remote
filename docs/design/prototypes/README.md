...
已有新版方向继续作为原型的约束：紧凑、图片优先的工作台；图像生产使用绿色，`LoRA` 训练使用玫瑰粉，两个模块同等重要；状态语义独立；明暗主题支持实时系统跟随、持久手动选择及明确重置。沿用已选择的 `PrimeReact v10 Styled` 及 `PrimeIcons`，不复制旧 `Demo` 组件体系。具体色值、字体文件、字号、尺寸和控件表现仍需本轮审核。
