...
test("image-heavy production pages use document navigation for primary route exits", () => {
  const hardLinkTargets = [
    ...
    ["src/app/projects/project-section-card-components.tsx", "href={`/projects/${projectId}/sections/${section.id}`}"],
    ["src/app/projects/project-section-card-components.tsx", "href={`/projects/${projectId}/sections/${section.id}/results`}"],
    ["src/app/projects/project-section-card-components.tsx", "href={section.latestRunId ? `/queue/${section.latestRunId}` : \"#\"}"],
