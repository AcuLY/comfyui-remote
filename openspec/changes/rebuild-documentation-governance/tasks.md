...
- [x] 11.1 将 `docs/workflow.api.json` 移到已批准的配置所有路径，更新加载器和每个测试与引用，并验证工作流模板行为完全一致；生产切换与删除旧副本后的只读工作流验证见 `evidence/2026-07-13-workflow-production-migration.md`。
- [x] 11.2 审计 `docs/**` 下其余非文档文件，把运行时输入、fixture 或生成输出迁移到实际配置、测试、数据或工件责任方。
- [x] 11.3 任何会改变运行时文件加载的迁移，都必须运行聚焦运行时、工作流生成、构建、队列和部署验证；不得把这些批次归类为仅文档。真实生产构建、双 worker 门、同端口重启、本地与公开验收证据见 `evidence/2026-07-13-workflow-production-migration.md`。
...
- [x] 12.6 实现并测试全部受治理 Markdown 语言门禁：正文和 frontmatter 人类可读字符串默认必须为简体中文，项目内兼容 Skill 也不例外；技术标识、命令、路径、协议字段、测试 fixture、数据载荷及 OpenSpec 解析器结构词必须按显式例外处理；增加正反 fixture，并让 `npm run docs:check` 阻止英文正文回归。
