...
### Requirement: 受治理文档默认使用简体中文
最终保留的全部受治理 Markdown `SHALL` 以简体中文书写正文、标题、表格说明，以及 frontmatter 中面向人的字符串；纳入仓库的项目 Skill 与兼容 Skill 也必须遵守。稳定技术形式是明确例外，包括技术标识、命令、路径、代码符号、路由、API/MCP/协议字段、测试 fixture、数据载荷，以及 OpenSpec 解析器所需结构词。例外不得成为保留整段英文解释正文的借口。

`npm run docs:check` 必须包含确定性、非写入、结构感知的语言门禁。它必须在全部受治理 Markdown 的正文节点与面向人 frontmatter 字段中检查语言；围栏或行内代码、链接目标、机器字段和已声明技术 token 必须按类别排除。英文解释正文回归必须以错误阻断本地 `full` 检查与 CI，并提供稳定路径、位置、证据、修复建议和责任方。实现不得依赖宽泛逐文件白名单。

#### Scenario: 受治理文档包含英文解释正文
- **WHEN** 受治理文档的正文或面向人 frontmatter 字段新增非例外英文句子
- **THEN** 文档门禁必须失败，并定位该句子与 owner
- **AND** 该违规解决前文档阶段不得验收

#### Scenario: 文档包含合法技术英文
- **WHEN** 英文只出现在技术标识、命令、路径、协议字段、代码、路由、测试 fixture、数据载荷或 OpenSpec 结构 token 中
- **THEN** 语言门禁不得要求翻译或改写该内容
- **AND** 同一文档中的人类解释正文仍必须为简体中文

#### Scenario: 项目 Skill 的人类说明受语言门禁保护
- **WHEN** `.codex/skills/**` 或 `.codebuddy/skills/**` 中已跟踪 Markdown 包含面向人的说明
- **THEN** 门禁必须像其他受治理 Markdown 一样检查其正文、标题和 frontmatter 人类字符串
- **AND** Skill 名称、命令、参数、脚本路径与技术栈 ID 继续保留稳定原文
