...
- 文档治理先于可观测性和工程规范；
- 全部受治理 Markdown 的正文与 frontmatter 人类可读字符串默认使用简体中文，并由确定性门禁阻止英文正文回归；纳入仓库的兼容 Skill 也在门禁范围内。
...
- 让最终保留的全部受治理 Markdown 以简体中文为默认人类语言，并为明确技术例外保留原始稳定形式。
...
- 翻译技术标识、命令、路径、协议字段、测试 fixture、数据载荷或 OpenSpec 解析器所需结构词；项目 Skill 的人类说明仍须中文化。
...
### 11. 受治理文档默认使用简体中文并由门禁保护

最终保留的全部受治理 Markdown，其正文、标题、表格说明及 frontmatter 中面向人的字符串必须使用简体中文，纳入仓库的项目 Skill 与兼容 Skill 也适用。稳定机器值不应为了中文化而改变：责任方与权威 ID、命令、路径、代码符号、路由、API/MCP/协议字段、测试 fixture、数据载荷以及 OpenSpec 解析器所需 `## ADDED Requirements`、`### Requirement:`、`#### Scenario:`、`WHEN`/`THEN`/`AND`/`BUT` 等结构词均是明确例外。
...
- 受治理 Markdown 的英文解释正文或 frontmatter 人类字符串回归时，语言门禁阻断；合法技术例外不能被自动改写。
...
12. 中文化最终保留的全部受治理 Markdown 并实现结构感知语言门禁；随后重建原地清单，清除每个确定性错误，为每个警告和语义问题项给出处置，并运行完整验证套件。
...
本修订版不再有未解决的信息架构或交接设计决策。用户已于 2026-07-12 授权 `apply`，并在本修订版把简体中文约束扩展到全部受治理 Markdown。`Apply` 期间，关于当前产品行为、部署策略、运行时归属、部分实现或语言例外边界的事实冲突，都必须走语义审计的用户决策路径，不能在实施中静默解决。
