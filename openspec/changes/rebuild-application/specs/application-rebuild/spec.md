...
### Requirement: 生产公共技术服务与图片执行语义分离

生产模块 SHALL 将 ComfyUI 通信、队列同步、文件传输和受控进程操作作为公共技术能力；图片专属适配器负责图片 Workflow 校验、参数注入、输出识别和图片结果落盘。GPU 协调继续归 shared 平台，保持当前单目标、单 GPU、训练优先和互斥/恢复规则。

#### Scenario: 提交并保存图片生成结果

- **WHEN** 执行当前 ProductionImageTask
- **THEN** 图片适配器校验并注入 Workflow，复用公共 ComfyUI 通信和文件传输，并负责图片输出识别、验收及结果持久化
- **AND** 公共服务不创建图片业务资产或决定图片任务的完成语义，图片 Task/Attempt 仍遵循全有或全无规则

#### Scenario: 协调当前 GPU 执行与恢复

- **WHEN** 图片任务或 TrainingRun 请求执行，或触发既定 GPU 恢复条件
- **THEN** shared 协调根据当前权威占用状态作决定，受控 ComfyUI 重启调用生产公共进程服务，保留既有优先级、互斥和恢复规则
- **AND** 未来执行类型须明确接入这一边界，本版不实现视频调度或多目标执行
