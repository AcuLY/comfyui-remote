...
- 训练资料字段 `productionPrompt` 尚未确认改名，继续保留。`Training*` 前缀简化不等于该字段改名。
- 通用组织与图片专属 API 的类型语义已确认；需要新增或拆分的精确路由后缀与公共小节数据结构仍需具体设计，本轮不预建视频协议。

## 来源与联动

用户在本轮对话中依次确认了公共 Project/Template 与小节分型、任务与 Attempt、结果与 Artifact、公共文件夹、图片配置及附属对象、项目级操作、模块路由，并最终要求训练实体前缀统一简化、每轮确认后及时更新文档。本文件只归纳这些确认，未把前期审视报告的全部建议视为批准。
