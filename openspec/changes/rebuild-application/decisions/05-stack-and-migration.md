...
| `MIG-03` | 新库转换 | 建立干净的 Drizzle/SQLite schema，按前向兼容边界把原生产项目/模板及文件夹映射为 ProductionProject、ProductionTemplate、ProductionProjectFolder、ProductionSectionFolder、ProductionTemplateSectionFolder。原图片小节的组织字段与图片配置分别映射到一对一 ProductionSection/ProductionImageSection；原模板小节同样拆分为 ProductionTemplateSection/ProductionTemplateImageSection，保留组织顺序及配置关联。图片任务/Attempt/结果/资产及预制等映射为对应 ProductionImage* 模型；原 LoraTraining* 目标模型统一为 Training*。完整转换层级/顺序、配置、图片关系、角色文本、Caption、用途标记、任务历史和模型元数据，记录必要的旧新实体名、ID 和路径映射；历史证据与原始备份仍用真实旧名，不让新版依赖旧表、旧 ORM 或兼容别名。生产与训练的预制/模板分别拥有；项目与模板配置协议复用但记录独立，应用时深复制 |
...
| `MIG-05` | 文件与配置 | 新版按 APP_DATA_ROOT、EXPORT_ROOT 和目标训练数据根及各类型资产归属组织受管文件；生产图片、应用侧训练图片、图片 Workflow 和图片交付包使用 FC-13/14 已确认目录，并转换数据库文件引用。内部项目目录使用稳定 projectId，交付包外层使用项目名称、ZIP 文件名使用 slug。ComfyUI 模型继续映射其实际目录，训练执行工作区仍按 LE-05；不因本轮文档更新移动实际文件。旧配置中的有效路径/参数属于需核对的数据，按新版设置合同导入；原 productionPrompt 文本显式映射到 imageProductionPrompt，内容与可空规则不变，不在新版维护两份字段或兼容别名。凭据继续按独立配置保存，不继承旧配置加载代码。node_modules、构建目录和可再生缓存不属于需保留的业务文件 |
