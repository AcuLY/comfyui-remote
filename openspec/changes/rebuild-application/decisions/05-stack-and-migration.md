...
| `MIG-05` | 文件与配置 | 新版按 APP_DATA_ROOT、EXPORT_ROOT 和目标训练数据根及各类型资产归属组织受管文件，转换数据库文件引用；ComfyUI 模型继续映射其实际目录，不因重建应用删改模型。旧配置中的有效路径/参数属于需核对的数据，按新版设置合同导入；productionPrompt 字段保持原名。凭据继续按独立配置保存，不继承旧配置加载代码。node_modules、构建目录和可再生缓存不属于需保留的业务文件 |
