...
| ID | 功能点 | 当前入口与行为 | 状态 | 主要依赖 / 移除影响 | 代码证据 | 决策 |
| --- | --- | --- | --- | --- | --- | --- |
| `GEN-WF-001` | 标准 workflow 配置 | 从版本化 JSON 加载 ComfyUI graph | 后台核心 | `config/workflows/standard-workflow.api.json` | workflow-prompt-builder | 保留版本化图片基础 Workflow，由生产模块的图片能力选择当前版本，不允许小节各自选择文件。校验、图片参数注入及原始/调试版本处理归图片适配器，公共 ComfyUI 技术服务不承担图片节点语义 |
...
| `GEN-EXEC-002` | ComfyUI 提交 | 验证 graph、提交 prompt、保存 promptId | 后台核心 | ComfyUI HTTP | run-executor/comfyui-service | 保留并重做：图片调度只提交 `unsubmitted` 任务；图片适配器负责 Workflow 校验和参数注入，公共 ComfyUI 服务负责实际通信提交。每次提交先创建内部 Attempt，ComfyUI 可达且满足既有 GPU 协调条件时幂等、有序提交，接收 promptId 后写入该 Attempt 并原子地把任务转为 `submitted`；不可达时保持 `unsubmitted`，不提示启动 ComfyUI；训练结束后按原规则恢复调度 |
| `GEN-EXEC-003` | 队列位置与历史轮询 | 等待开始、轮询完成/失败、有限并发 | 后台核心 | ComfyUI queue/history | run-executor | 生产公共 ComfyUI 服务按 promptId 接收 WebSocket execution/progress/completion/error 事件；存在 submitted/running 任务时固定每 1 秒查询一次 queue/history 并分发对账结果，WebSocket 断线也不改变频率。图片执行逻辑维护自身 Task/Attempt 的状态、submittedAt、startedAt、finishedAt、排队/生成耗时和错误，Task 按 `ARCH-010` 累计各阶段。图片 Attempt 不建模部分成功，全部图片结果持久化后才 completed |
| `GEN-EXEC-004` | 输出下载与持久化 | 下载图片、写 ImageResult 和受管文件 | 后台核心 | ComfyUI output、`data/images` | image-result-service | 图片适配器负责输出识别、图片校验、受管路径及结果/资产落盘，文件收发复用公共 ComfyUI 传输能力；公共传输服务不创建图片业务记录或决定其生命周期。全部预期图片下载、校验和落盘成功后才创建正式 ProductionImageResult；任一失败则图片任务失败并清理本次临时文件，不产生“部分成功” |
...
| `GEN-EXEC-009` | ComfyUI 队列缓存 | 短时缓存 queue snapshot，减少轮询压力 | 后台 | 内存缓存 | comfyui-service | 新版移除独立队列快照缓存：由生产公共 ComfyUI 服务的唯一后台同步循环按 IP-08 每 1 秒读取一次 queue/history 并分发结果，避免各适配器重复轮询；业务权威状态仍由各自 Task/Attempt 负责，本版仅接入已确认图片执行 |
