...
确认日期：2026-09-05。新应用尚未实施，页面设计待用户逐页审核。
...
| 模块 | 图像生产与 LoRA 训练完全对等，平台能力按实际名称暴露 | MCP、专用 Agent 写接口、/api/shared 命名空间 |
| 图像生产任务 | Task + Attempt；ComfyUI 自有队列可接收多个 Task；全有或全无输出 | 单任务提交上限、缓冲队列实体、失败重试新建 Task |
